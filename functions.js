  ...
  for (let point of points) {
    ...
    if (xNeighbor && yNeighbor && zNeighbor) {
      ...
      const midpoint = p5.Vector.add(
        point,
        createVector(cellSize / 2, cellSize / 2, cellSize / 2),
      );
      ...
      const facesColors = [];
      cells.push(
        new Cell(midpoint, edges, cellIndex, getRandomColors(BECcolors, 6)[0]),
      );
      cellIndex++;
    }
  }
...
function createInfos() {
  const titre = "HABITER";
  const sTitre =
    "Expositions, rencontres, ateliers autour \ndes arts numériques et hybrides";
...
function loadInputs(onReady) {
  ...
  // at the very end, once everything is wired up:
...
// Shared by both projects. Requires #progress-bar-fill, #loading-label,
// and #loading-screen in the HTML.
